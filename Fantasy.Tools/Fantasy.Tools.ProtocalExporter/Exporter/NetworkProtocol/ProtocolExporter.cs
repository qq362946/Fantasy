...
public sealed class ProtocolExporter
{
    ...
    public readonly CustomSerialize _customSerialize;

    public ProtocolExporter(CustomSerialize selectSerializeType)
    {
        Console.OutputEncoding = Encoding.UTF8;

        _customSerialize = selectSerializeType;
    ...
    private async Task Start(NetworkProtocolOpCodeType opCodeType)
    {
        ...
        foreach (var filePath in protocolFiles)
        {
            ...
            string protocolMember = _customSerialize.Member;
            string protocolType = _customSerialize.Attribute;
            string protocolIgnore = _customSerialize.Ignore;
            var protocolOpCodeType = OpCodeProtocolType.Protocal;
            ...
            foreach (var line in fileText.Split('\n'))
            {
                ...
                switch (currentLine)
                {
                    ...
                    case "}":
                    {
                        ...
                        errorCodeStr.Clear();
                        keyIndex = 1;
                        protocolType = _customSerialize.Attribute;
                        protocolIgnore = _customSerialize.Ignore;
                        protocolMember = _customSerialize.Member;
                        protocolOpCodeType = OpCodeProtocolType.Protocal;
                        continue;
                    }
                    case "":
                    {
