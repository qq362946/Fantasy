...
业务入口是 `session.GetOrCreateRoaming(roamingId, delayRemove)`。同一 `roamingId` 已存在时自动换绑新 Session；调用方不再区分创建和重连。
...
**重连窗口**：Session 断开后先停止转发，再按 `delayRemove` 保留上下文；期限内重连会取消销毁并恢复转发。动态 Gate 下由新的 `SessionRoamingComponent.RuntimeId` 接管目标 Terminus，旧 owner 的延迟请求不会影响新连接。
