---
name: fantasy-net
description: Development and code review guide for Fantasy / Fantasy.Net / Fantasy.Unity distributed game server framework (C#). Use this skill when users mention Fantasy in contexts of game servers, Unity clients, ECS, distributed scenes, hot reload, or network architecture, or when they mention FTask, Scene, SubScene, Entity, Component, System, OnCreateScene, Timer, Event, EventAwaiter, SphereEvent, cross-server events, SphereEventComponent, SphereEventArgs, SphereEventSystem, remote subscription, pub/sub, PublishToRemoteSubscribers, Subscribe, network message Handlers, proto/Outer/Inner protocols, protocol export, Address, Addressable, Roaming, Terminus, cross-server transfer, HTTP server, HTTP Controller, OnConfigureHttpServices, OnConfigureHttpApplication, SceneContextFilter, Fantasy.config, machine/process/world/scene/idFactory/network/session/database, MongoDB, scene.World.Database, SeparateTable, logging, server project creation/integration, Unity connection and Session, etc. Also use this skill when users request review, code review, check code, look for issues, check Fantasy compliance, identify risks, best practices, troubleshoot, or find what's wrong. Even if users don't explicitly say "Fantasy", use this skill when the requirement clearly involves writing, reviewing, or troubleshooting Fantasy framework-related entities, messages, cross-server events, configuration, database, HTTP services, client connections, server architecture, or runtime system code.
---
