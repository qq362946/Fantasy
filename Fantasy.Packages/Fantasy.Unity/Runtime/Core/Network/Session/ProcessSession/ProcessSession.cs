...
public sealed class ProcessSession : Session
{
    ...
    public override void Send(MemoryStreamBuffer memoryStream, Type messageType, uint protocolCode = 0, uint rpcId = 0, long address = 0)
    {
        if (IsDisposed)
        {
            return;
        ...
        this.Scheduler(messageType, rpcId, address, protocolCode, memoryStream);
    }
    ...
    public object Deserialize(Type messageType, MemoryStreamBuffer memoryStream, ref OpCodeIdStruct opCodeIdStruct)
    {
