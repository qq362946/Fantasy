...
/// </summary>
internal sealed class I_RevokeRemoteSubscriberRequestHandler : RouteRPC<Scene, I_RevokeRemoteSubscriberRequest, I_RevokeRemoteSubscriberResponse>
{
    protected override async FTask Run(Scene scene, I_RevokeRemoteSubscriberRequest request, I_RevokeRemoteSubscriberResponse response, Action reply)
    {
        ...
        {
            response.ErrorCode = InnerErrorCode.ErrRevokeRemoteSubscriberInvalidRouteId;
            return;
        }
        ...
        await scene.SphereEventComponent.Unsubscribe(request.RouteId, request.TypeHashCode, false);
    }
}
#endif