...
/// 用于远程节点订阅本地场景的领域事件。
/// </summary>
internal sealed class I_SubscribeSphereEventRequestHandler : RouteRPC<Scene, I_SubscribeSphereEventRequest, I_SubscribeSphereEventResponse>
{
    protected override async FTask Run(Scene scene, I_SubscribeSphereEventRequest request, I_SubscribeSphereEventResponse response, Action reply)
    {
        ...
        {
            response.ErrorCode = InnerErrorCode.ErrSubscribeSphereEventInvalidRouteId;
            return;
        }
        ...
        scene.SphereEventComponent.RegisterRemoteSubscriber(request.RouteId, request.TypeHashCode);
