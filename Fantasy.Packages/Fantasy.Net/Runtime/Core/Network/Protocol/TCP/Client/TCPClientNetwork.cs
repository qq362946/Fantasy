...
namespace Fantasy.Network.TCP
{
    public sealed class TCPClientNetwork : AClientNetwork
    {
        ...
        public override Session Connect(string remoteAddress, Action onConnectComplete, Action onConnectFail, Action onConnectDisconnect, bool isHttps, int connectTimeout = 5000)
        {
            ...
            SocketAsyncEventArgs outArgs = null;
            Session session;
            ...
            try
            {
                connectPending = _socket.ConnectAsync(outArgs);
            }
            catch (Exception e)
            {
                outArgs.Dispose();
            ...
            if (!connectPending)
            {
                var socketError = outArgs.SocketError;
                outArgs.Dispose();
        ...
        private void Send()
        {
            if (_isSending || IsDisposed || !_isConnected)
            {
                return;
            }
