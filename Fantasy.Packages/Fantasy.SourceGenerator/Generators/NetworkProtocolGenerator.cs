...
internal partial class NetworkProtocolGenerator : IIncrementalGenerator
{
    ...
    {
        var networkProtocolTypeInfoList = networkProtocolTypeInfos.Where(d=>!d.IsDataClass).ToList();
        // 生成网络网络协议类型注册的类。
        ...
        GenerateProtoBufDispatcherCode(context, compilation, networkProtocolTypeInfos);
    }
        ...
        List<NetworkProtocolTypeInfo> networkProtocolTypeInfoList)
    {
        ...
        // 过滤掉纯数据类，只保留协议消息类型，并且有 ResponseType 的
        var requestList = networkProtocolTypeInfoList
            .Where(d => d.ResponseType != null)
            .ToList();
        var builder = new SourceCodeBuilder();
        // 添加文件头
        builder.AppendLine(GeneratorConstants.AutoGeneratedHeader);
    ...
    private static void GenerateProtoBufDispatcherCode(
        SourceProductionContext context,
        Compilation compilation,
        IEnumerable<NetworkProtocolTypeInfo> networkProtocolTypeInfoList)
    {
        // 处理所有有 ProtoContract 特性的类型（包括协议消息和纯数据类）
    ...
    private static NetworkProtocolTypeInfo? GetNetworkProtocolInfo(GeneratorSyntaxContext context)
    {
        ...
        // 判断是否是纯数据类（没有 OpCode 方法的 AMessage 子类）
        bool isDataClass = !opCodeValue.HasValue;

        return new NetworkProtocolTypeInfo(
            symbol.GetFullName(),
            opCodeValue ?? 0, // 纯数据类的 OpCode 为 0
            responseTypeName,
            routeTypeValue,
            hasProtoContract,
            isDataClass
