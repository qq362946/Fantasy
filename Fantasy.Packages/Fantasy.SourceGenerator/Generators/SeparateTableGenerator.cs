...
namespace Fantasy.SourceGenerator.Generators
{
    ...
    public partial class SeparateTableGenerator : IIncrementalGenerator
    {
        private static readonly DiagnosticDescriptor MissingPartialRule = new(
            "FANTASY009",
            "SeparateTable entity must be partial",
            "SeparateTable entity '{0}' must be declared partial so the source generator can add ISupportedSeparateTable",
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(compilationAndTypes, static (spc, source) =>
            {
                ...
                GenerateSupportedSeparateTableCode(spc, source.Right!);
            });
        }
        ...
        private static SeparateTableTypeInfo? GetSeparateTableTypeInfo(GeneratorSyntaxContext context)
        {
            ...
            return separateTableInfo.Any() ? SeparateTableTypeInfo.Create(symbol, classDecl, separateTableInfo) : null;
        }
        ...
        private static void GenerateSupportedSeparateTableCode(
            SourceProductionContext context,
            IEnumerable<SeparateTableTypeInfo> separateTableTypeInfos)
        {
            var separateTableTypeInfoList = separateTableTypeInfos
                .Where(info => !info.ImplementSupportedSeparateTable)
                .ToList();
            ...
            {
                if (!separateTableTypeInfo.IsPartial)
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        MissingPartialRule,
                        separateTableTypeInfo.Location,
                        separateTableTypeInfo.TypeName));
                    continue;
                }
                ...
                if (!string.IsNullOrEmpty(separateTableTypeInfo.NamespaceName))
                {
                    builder.BeginNamespace(separateTableTypeInfo.NamespaceName);
                }

                builder.AppendLine($"{separateTableTypeInfo.Accessibility} partial class {separateTableTypeInfo.TypeDeclarationName} : global::Fantasy.Entitas.Interface.ISupportedSeparateTable");
                builder.OpenBrace();
                builder.CloseBrace();
                ...
                context.AddSource($"{separateTableTypeInfo.HintName}.ISupportedSeparateTable.g.cs", builder.ToString());
            }
        }
        ...
        private static bool IsSeparateTableClass(SyntaxNode node)
        {
            ...
            foreach (var attributeList in classDecl.AttributeLists)
            {
                foreach (var attribute in attributeList.Attributes)
                {
                    ...
                    // 3. [Fantasy.SeparateTable.SeparateTable(...)]
                    // 4. [Fantasy.SeparateTable.SeparateTableAttribute(...)]
                    // 5. [global::Fantasy.SeparateTable.SeparateTable(...)]
                    if (attributeName == "SeparateTable" ||
                        attributeName == "SeparateTableAttribute" ||
                        attributeName == "Fantasy.SeparateTable.SeparateTable" ||
                        attributeName == "Fantasy.SeparateTable.SeparateTableAttribute" ||
                        attributeName == "global::Fantasy.SeparateTable.SeparateTable" ||
                        attributeName == "global::Fantasy.SeparateTable.SeparateTableAttribute" ||
        ...
        private sealed class SeparateTableTypeInfo
        {
            ...
            public readonly string TypeDeclarationName;
            public readonly string NamespaceName;
            public readonly string Accessibility;
            public readonly string HintName;
            public readonly bool IsPartial;
            public readonly bool ImplementSupportedSeparateTable;
            public readonly Location Location;

            private SeparateTableTypeInfo(string typeFullName, string typeName,
                string typeDeclarationName, string namespaceName, string accessibility,
                string hintName, bool isPartial, bool implementSupportedSeparateTable,
                Location location, Dictionary<string, string> separateTableInfo)
            {
                TypeFullName = typeFullName;
                TypeName = typeName;
                TypeDeclarationName = typeDeclarationName;
                NamespaceName = namespaceName;
                Accessibility = accessibility;
                HintName = hintName;
                IsPartial = isPartial;
                ImplementSupportedSeparateTable = implementSupportedSeparateTable;
                Location = location;
                ...
                Dictionary<string, string> separateTableInfo)
            {
                return new SeparateTableTypeInfo(
                    symbol.GetFullName(),
                    symbol.Name,
                    GetTypeDeclarationName(symbol),
                    GetNamespaceName(symbol),
                    GetAccessibility(symbol),
                    GetHintName(symbol),
                    classDeclaration.Modifiers.Any(SyntaxKind.PartialKeyword),
                    symbol.ImplementsInterface("Fantasy.Entitas.Interface.ISupportedSeparateTable"),
                    classDeclaration.Identifier.GetLocation(),
                    separateTableInfo);
            }

            private static string GetTypeDeclarationName(INamedTypeSymbol symbol)
            {
                ...
                return $"{symbol.Name}<{string.Join(", ", symbol.TypeParameters.Select(type => type.Name))}>";
            }
