...
namespace Fantasy.Unity
{
    ...
    public class AssetBundleUpdate
    {
        ...
        private AssetBundleComponent _assetBundleComponent;
        private AssetBundleVersionInfo _localVersionInfo;
        private readonly HashSet<AssetBundleVersion> _updateFiles = new();

        public AssetBundleUpdate(AssetBundleComponent assetBundleComponent, Download download)
        {
            _download = download;
            _assetBundleComponent = assetBundleComponent;
            _platformName = assetBundleComponent.GetPlatform();
            _localVersionMD5 = $"{AppDefine.RemoteAssetBundlePath}/{AppDefine.VersionMD5Name}";
            _localVersionName = $"{AppDefine.RemoteAssetBundlePath}/{AppDefine.VersionName}";
        }

        public async FTask<AssetBundleUpdateState> CheckVersionMD5()
        {
            _download.Clear();
            _remotePath = $"{AppDefine.RemoteUpdatePath}{_platformName}";

            try
            {
                var remoteVersionMD5Url = $"{_remotePath}/{AppDefine.VersionMD5Name}";
        ...
        public async FTask<AssetBundleUpdateState> DownloadVersion()
        {
            ...
            {
                _download.Clear();
                var remoteVersionName = await _download.DownloadByte($"{_remotePath}/{AppDefine.VersionName}");
                ...
                _updateFiles.Clear();
                _assetBundleComponent.NeedUpdateSize = 0;
                var deleteFiles = new HashSet<AssetBundleVersion>();
                var remoteVersionInfo = ProtoBuffHelper.FromBytes<AssetBundleVersionInfo>(remoteVersionName);

                if (File.Exists(_localVersionName))
                {
                    var localVersionNameBytes = await File.ReadAllBytesAsync(_localVersionName);
                    _localVersionInfo = ProtoBuffHelper.FromBytes<AssetBundleVersionInfo>(localVersionNameBytes);
                    ...
                    foreach (var assetBundleVersion in _localVersionInfo.List)
                    {
                        ...
                        _assetBundleComponent.NeedUpdateSize += remoteAssetBundleVersion.Size;
                ...
                {
                    _assetBundleComponent.NeedUpdateSize += assetBundleVersion.Size;
        ...
        public async FTask<AssetBundleUpdateState> DownloadAssetBundle()
        {
            try
            {
                ...
                File.WriteAllBytes(_localVersionName, ProtoBuffHelper.ToBytes(_localVersionInfo));
                return AssetBundleUpdateState.Complete;
            }
            catch (Exception e)
            {
        ...
        private async FTask DownloadAssetBundle(AssetBundleVersion assetBundleVersion)
        {
            ...
            var savePath = $"{AppDefine.RemoteAssetBundlePath}/{assetBundleVersion.Name}";
            var saveDirectory = assetBundleVersion.Name.Split('/');
            var currentDirectory = AppDefine.RemoteAssetBundlePath;
