...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        private void ExecuteBuild()
        {
            ...
            var newManifestPath = $"{m_UserData.m_OutputPath}/{AppDefine.AssetBundleManifestName}.manifest";
            File.Move(oldManifestPath,newManifestPath);
            // 计算所有文件的MD5
            var versionPath = $"{m_UserData.m_OutputPath}/{AppDefine.VersionName}";
            var versionMD5Path = $"{m_UserData.m_OutputPath}/{AppDefine.VersionMD5Name}";
            ...
            {
                File.Move(currentManifest, $"{m_UserData.m_OutputPath}/{AppDefine.AssetBundleManifestName}");
            }
            var versionBytes = AssetBundleComponent.CalculateMD5(m_UserData.m_OutputPath, m_IncludeManifest.state);
            File.WriteAllBytes(versionPath, versionBytes);
            File.WriteAllBytes(versionMD5Path, Encoding.UTF8.GetBytes(EncryptHelper.FileMD5(versionPath)));
