...
public sealed class KCPServerNetwork : ANetwork
{
    ...
    private void SendAsync(byte[] buffer, int offset, int count, EndPoint endPoint)
    {
        try
        {
            _socket.SendToAsync(new ArraySegment<byte>(buffer,offset,count), SocketFlags.None, endPoint);
        }
