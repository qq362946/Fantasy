...
namespace Fantasy
{
    public static class DestroySystemCodeGenerator
    {
        private const string SurroundXml = "---OnDestroy---";
        private const string OutSurroundXml = "---DestroySystem---";

        public static bool CheckExistOldCode(string declarationName, List<string> oldCode, out int[] destroySystemIdx)
        {
            destroySystemIdx = new[] { -1, -1 };
            for (var i = 0; i < oldCode.Count; i++)
            {
                var oldScriptLine = oldCode[i];
                if (destroySystemIdx[0] == -1 && Regex.IsMatch(oldScriptLine, @$"//\s*<{OutSurroundXml} class='{declarationName}'>"))
                    destroySystemIdx[0] = i;
                if (destroySystemIdx[0] > -1 && destroySystemIdx[1] == -1 && Regex.IsMatch(oldScriptLine, @$"//\s*<{OutSurroundXml}/>"))
                    destroySystemIdx[1] = i;
            }

            return destroySystemIdx[0] > -1 && destroySystemIdx[1] > destroySystemIdx[0];
        }
        ...
        public static List<string> GenCode(string declarationName, List<string> bindingCode)
        {
            ...
            newCode.Add($"public sealed class {declarationName}DestroySystem : DestroySystem<{declarationName}>");
            newCode.Add("{");
            newCode.Add($"protected override void Destroy({declarationName} self)");
            newCode.Add("{");
            ...
            Dictionary<IBindingCodeGenerator, Dictionary<string, List<string>>> oldPropBindings)
        {
            ...
            foreach (var bindingCodeGenerator in bindingCodeGenerators)
            {
                ...
                foreach (var fieldName in fieldNameSet)
                {
                    ...
                    var code = bindingCodeGenerator.GetUnBindingCode(targetField, functionName);
                    if (code.Count == 0)
                        continue;
        ...
        public static bool ValidateOldCode(List<string> oldCode, out int[] destroyIdx)
        {
            destroyIdx = new[] { -1, -1 };
            ...
            {
                var line = oldCode[i];
                if (destroyIdx[0] == -1 && Regex.IsMatch(line, @$"//\s*<{SurroundXml}>"))
                    destroyIdx[0] = i;
                if (destroyIdx[0] > -1 && destroyIdx[1] == -1 && Regex.IsMatch(line, @$"//\s*<{SurroundXml}/>"))
                    destroyIdx[1] = i;
            }

            return destroyIdx[0] != -1 && destroyIdx[1] > destroyIdx[0];
        }
