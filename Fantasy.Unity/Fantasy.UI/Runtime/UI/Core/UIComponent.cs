    ...
    public sealed class UIComponent : Entity
    {
        ...
        private void UILayerInitialize(int designResolutionX, int designResolutionY, CanvasScaler.ScaleMode scaleMode, CanvasScaler.ScreenMatchMode screenMatchMode)
        {
            ...
            for (var i = 1; i < uiLayers.Length; i++)
            {
                ...
                var uiLayerComponent = Create<UILayerComponent>(Scene, true);
        ...
        public async FTask<T> CreateAsync<T>(UILayer layer = UILayer.None, bool isRunEvent = true) where T : UI, new()
        {
            ...
            try
            {
                ...
                // 异步加载AB包资源
                await Scene.AssetBundleComponent.LoadBundleAsync(ui.BundleName);
                var gameObject = Scene.AssetBundleComponent.GetAsset<GameObject>(ui.BundleName, ui.AssetName);
                // 实例化GameObject
                ...
                if (isRunEvent)
                {
                    Scene.EntityComponent.Awake(ui);
                    Scene.EntityComponent.StartUpdate(ui);
                }
        ...
        public T Create<T>(UILayer layer = UILayer.None, bool isRunEvent = true) where T : UI, new()
        {
            ...
            try
            {
                ...
                // 加载AB包资源
                Scene.AssetBundleComponent.LoadBundle(ui.BundleName);
                var gameObject = Scene.AssetBundleComponent.GetAsset<GameObject>(ui.BundleName, ui.AssetName);
                // 实例化GameObject
                ui.GameObject = Object.Instantiate(gameObject);
                // 设置UI的Layer
