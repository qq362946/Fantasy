...
namespace Fantasy
{
    public sealed class UpdateAssetBundle : MonoBehaviour, IDisposable
    {
        ...
        {
            Log.Debug($"热更资源存放路径:{Define.AppHotfixResPath}");
            
            _platformName = GetPlatform();
            _localVersionName = $"{Define.AppHotfixResPath}/{Define.VersionName}";
            _localVersionMD5 = $"{Define.AppHotfixResPath}/{Define.VersionMD5Name}";
            var hotfixResDir = Directory.GetDirectories(Define.AppHotfixResPath, "*", SearchOption.AllDirectories);
        ...
        private async FTask DownloadAssetBundle(AssetBundleVersion assetBundleVersion)
        {
            ...
            var savePath = $"{Define.AppHotfixResPath}/{assetBundleVersion.Name}";
            var saveDirectory = assetBundleVersion.Name.Split('/');
            var currentDirectory = Define.AppHotfixResPath;
