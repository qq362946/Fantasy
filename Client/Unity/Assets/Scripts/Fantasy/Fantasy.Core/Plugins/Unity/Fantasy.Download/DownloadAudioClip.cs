...
namespace Fantasy.Core
{
    ...
    {
        public FTask<AudioClip> StartDownload(string url, AudioType audioType, FCancellationToken cancellationToken = null)
        {
            var task = FTask<AudioClip>.Create(false);
            var unityWebRequestAsyncOperation = Start(UnityWebRequestMultimedia.GetAudioClip(Uri.EscapeUriString(url), audioType));
