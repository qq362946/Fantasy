...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        {
            var assetBundleManifest = m_UserData.m_BuildTarget.ToString();
            var assetBundleVersionInfoList = new AssetBundleVersionInfo();
            var sourceFiles = Directory.GetFiles(m_UserData.m_OutputPath, "*.*", SearchOption.AllDirectories);

            foreach (string filePath in sourceFiles)
            {
                if (!m_IncludeManifest.state && filePath.EndsWith(".manifest"))
                {
                    // 有AssetBundleManifest文件就够用了、其他包的Manifest文件不需要再计算和参与导出了
                ...
                if (assetBundleManifest == Path.GetFileNameWithoutExtension(path))
                {
                    // 给assetBundleManifest改名字
                    path = $"{m_UserData.m_OutputPath}/{Define.AssetBundleManifestName}";
                ...
                var assetBundleVersion = new AssetBundleVersion
                {
                    MD5 = MD5Helper.FileMD5(file),
                    Size = (ulong)file.Length,
                    Name = path.Substring(m_UserData.m_OutputPath.Length + 1)
                };

                assetBundleVersionInfoList.Add(assetBundleVersion);
            }
            
            var bytes = ProtoBufHelper.ToBytes(assetBundleVersionInfoList);
