...
public sealed class C2G_CreateChatRouteRequestHandler : MessageRPC<C2G_CreateChatRouteRequest, G2C_CreateChatRouteResponse>
{
    protected override async FTask Run(Session session, C2G_CreateChatRouteRequest request, G2C_CreateChatRouteResponse response, Action reply)
    {
        ...
        // 通过chatSceneConfig拿到这个Scene的Address
        var chatAddress = chatSceneConfig.Address;
        ...
        // 任何一个实体的RunTimeId都可以做为Address使用。
        // 所以这个传递了一个session.RunTimeId，是方便Chat发送消息回Gate上。
        var routeResponse = (Chat2G_CreateRouteResponse)await networkMessagingComponent.Call(chatAddress,
            new G2Chat_CreateRouteRequest()
            {
                GateAddress = session.Address
            });
        ...
        // routeResponse会返回一个ChatAddressId，这个就是Chat的Address。
        routeComponent.AddAddress((int)RouteType.ChatRoute,routeResponse.ChatAddress);
