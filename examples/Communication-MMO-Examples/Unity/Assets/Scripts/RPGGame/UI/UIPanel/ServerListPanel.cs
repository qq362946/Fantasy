...
public class ServerListPanel : BasePanel
{
    ...
    {
        int regionId = 0;
        foreach(var zone in GameManager.Ins.zoneInfo)
        {
            if(regionId!=zone.RegionId){
                regionId++;
                var go = UIFacade.Instance.GetUI("zoneTabSlot","zoneTabSlot"+regionId);
    ...
    void CreateZoneSlot()
    {
        foreach(var zone in GameManager.Ins.zoneInfo)
        {
            ...
            var go = UIFacade.Instance.GetUI("zoneSlot","zoneSlot"+zone.ZoneId);
            ...
            if(zone.ZoneId == (selectZoneId==0?GameManager.Ins.defaultZone.ZoneId:selectZoneId)) 
                slot.btn.Select();
            
            slot.btn.onClick.SetListener(() => {
                selectZoneId = slot.zoneId;
                GameManager.Ins.selectZone = zone;
    ...
    void ClearList()
    {
        ...
        {
            UIFacade.Instance.PushUI("zoneSlot"+slot.zoneId,slot.gameObject);
        }
    }
}