...
namespace Fantasy
{
    public static class MemoryPackHelper
    {
        ...
        {
            var @object = MemoryPackSerializer.Deserialize<T>(new Span<byte>(bytes, index, count));
            if (@object is ASerialize aSerialize)
            {
                aSerialize.AfterDeserialization();
            }
        ...
        {
            var @object = MemoryPackSerializer.Deserialize(type, new Span<byte>(bytes, index, count));
            if (@object is ASerialize aSerialize)
            {
                aSerialize.AfterDeserialization();
            }
